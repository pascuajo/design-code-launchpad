...
export interface FontSetting {
  component: string;
  elementType: 'h1' | 'h2' | 'h3' | 'h4' | 'p' | 'span' | 'li' | 'label' | 'button' | 'pill' | 'formField' | 'highlighted';
  fontFamily: string;
  isActive: boolean;
}

export interface ComponentFonts {
  [componentId: string]: {
    [elementType: string]: FontSetting;
  };
}

// Default font configuration
export const defaultFonts: ComponentFonts = {
  header: {
...
export let currentFonts: ComponentFonts = { ...defaultFonts };

// Function to update fonts (called by FontManager)
export function updateFonts(newFonts: ComponentFonts) {
...
export function getFont(componentId: string, elementType: string): string {
...
function applyFontsToDOM(fonts: ComponentFonts) {
...
function getComponentClasses(componentId: string): string[] {
