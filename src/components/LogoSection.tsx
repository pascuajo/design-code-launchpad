...
export function LogoSection() {
  ...
  return <section className="w-full bg-yellow-300 py-16 px-4">
      <div className="max-w-6xl mx-auto">
        <AnimateOnScroll>
          <div className="flex flex-row justify-center items-center gap-4 md:gap-8 lg:gap-12 flex-wrap">
            {logos.map(logo => <div key={logo.id} className="flex-shrink-0">
                <img src={logo.imageUrl} alt={`${logo.name} logo`} className={`${logo.className} w-auto object-contain`} />
              </div>)}
          </div>
        </AnimateOnScroll>
      </div>
